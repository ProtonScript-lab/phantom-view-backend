...
class GigaChatService {
  ...
  async getAccessToken() {
    ...
    try {
      ...
      this.tokenExpires =
        Date.now() + (response.data.expires_in * 1000 - 10000)
  ...
  async generatePostIdeas(topic) {
    ...
    const prompt = `
Придумай 5 идей для постов на тему "${topic}".
Каждая идея должна быть краткой и цепляющей.
Ответ только списком через дефис.
`
  ...
  async getTrends() {
    ...
    const prompt = `
Какие темы сейчас популярны в соцсетях по категориям:
