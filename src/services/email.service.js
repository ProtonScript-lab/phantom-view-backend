...
class EmailService {
  constructor() {
    const port = parseInt(process.env.EMAIL_PORT) || 587; // используем 587 по умолчанию (STARTTLS)
    ...
    this.transporter = nodemailer.createTransport({
      host,
      port,
      secure: port === 465, // true для 465, false для 587 (STARTTLS)
      auth: {
  ...
  async sendVerificationCode(to, code) {
    ...
    try {
      logger.info(`Попытка отправки email на ${to} через ${this.transporter.options.host}:${this.transporter.options.port}`);
      const info = await this.transporter.sendMail(mailOptions);
      logger.info('Email отправлен', { to, messageId: info.messageId });
      return info;
    } catch (error) {
      logger.error('Ошибка отправки email', { to, error: error.message, stack: error.stack });
